...
# mnist sweep for gale_scaffold_r
  ...
  for ridge_alpha in 0.1 1.0 10.0; do
    python -m appfl_sim.runner \
      --config appfl_sim/config/cross-silo/mnist/gale_scaffold_r.yaml \
        ...
        "logging.configs.wandb_tags=cross-silo,mnist,gale_scaffold_r,sweep" \
        ...
        logging.name="sweep_mnist_gale_scaffold_r_${lr_decay}_${ridge_alpha}" &
  done
  wait
done
