...
SWEEP_NAME="${SWEEP_NAME:-sweep_fedadam_gale_nc}"
RUN_CAP="${RUN_CAP:-60}"
...
launcher_program="${tmpdir}/sweep_fedadam_gale_nc_launcher.py"
...
cmd = [
    ...
    "appfl_sim.runner",
    "--config",
    "appfl_sim/config/cross-device/londonsmartmeter/gale_adam.yaml",
    ...
    "logging.configs.wandb_tags=cross-device,londonsmartmeter,gale_adam,sweep,bayes",
    "optimizer.lr=0.001",
    ...
    "logging.name="
    + f"sweep_fedadam_gale_nc_{args.discount_gamma:.2f}_{args.exploration_alpha:g}_{args.lr_decay_gamma:.2f}",
]

raise SystemExit(subprocess.call(cmd))
PY

sweep_config="${tmpdir}/sweep_fedadam_gale_nc.yaml"
