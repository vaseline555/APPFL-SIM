...
def _resolve_loss_kwargs(config: DictConfig | dict) -> Dict[str, Any]:
    legacy_kwargs = _cfg_to_dict(_cfg_get(config, "loss.configs", {}))
    explicit_kwargs = _cfg_to_dict(_cfg_get(config, "loss.loss_kwargs", {}))
    merged_kwargs = dict(legacy_kwargs)
...
def build_loss_from_config(config: DictConfig | dict):
    ...
    kwargs = _resolve_loss_kwargs(config)
    if backend == "torch":
...
def build_loss_from_train_cfg(train_cfg: DictConfig | dict):
    payload = {
        "loss": {
            ...
            "loss_kwargs": _cfg_get(train_cfg, "loss.loss_kwargs", {}),
        }
    }
...
def _ensure_model_cfg(cfg: DictConfig) -> None:
    ...
    if _cfg_get(cfg, "loss.loss_kwargs", None) is None:
        _cfg_set(cfg, "loss.loss_kwargs", OmegaConf.create({}))
...
def _build_train_cfg(
    ...
    loss_kwargs = _resolve_loss_kwargs(config)
