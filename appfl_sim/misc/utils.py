...
def clone_state_dict(state_dict: OrderedDict) -> OrderedDict:
    return OrderedDict((k, v.detach().clone()) for k, v in state_dict.items())
...
def resolve_rank_device(
    ...
    del world_size  # world_size reserved for future placement strategies.
    ...
    if ":" in base:
        # Respect explicit cuda index when provided (e.g., cuda:0).
        suffix = base.split(":", 1)[1].strip()
        if suffix and suffix.isdigit():
            return f"cuda:{int(suffix)}"
        # `cuda:local` or malformed index -> map by local rank.
...
def get_appfl_algorithm(
    ...
    try:
        appfl_module = importlib.import_module("appfl_sim.algorithm")
...
def get_appfl_authenticator(
    ...
    try:
        appfl_module = importlib.import_module("appfl_sim.login_manager")
