...
def _load_torchvision_model(spec: ModelSpec):
    ...
    norm_name = kwargs.get("norm_layer", None)
    if isinstance(norm_name, str):
        norm_key = norm_name.strip().lower()
        if norm_key in {"groupnorm", "group_norm"}:
            num_groups = _safe_int(kwargs.pop("norm_num_groups", 2), 2)
            eps = _safe_float(kwargs.pop("norm_eps", 1e-5), 1e-5)
            affine = _safe_bool(kwargs.pop("norm_affine", True), True)
            kwargs.pop("norm_layer", None)

            def _group_norm(num_features: int):
                if int(num_features) % int(num_groups) != 0:
                    raise ValueError(
                        f"GroupNorm requires num_features divisible by num_groups. "
                        f"Got num_features={num_features}, num_groups={num_groups}."
                    )
                return torch.nn.GroupNorm(
                    num_groups=int(num_groups),
                    num_channels=int(num_features),
                    eps=float(eps),
                    affine=bool(affine),
                )

            kwargs["norm_layer"] = _group_norm
