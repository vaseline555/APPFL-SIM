...
def preprocess(root):
    ...
    def _convert_to_json(path, raw_all, indices):
        ...
        # refine raw data
        # Keep `text` as object dtype because we store token-id lists per row.
        raw_all['target'] = raw_all['target'].replace({4: 1, 2: 0})
        raw_all['text'] = _cleanse(raw_all['text']).astype('object')
        raw_all['text'] = raw_all['text'].map(lambda x: _line_to_indices(x, indices)).astype('object')
