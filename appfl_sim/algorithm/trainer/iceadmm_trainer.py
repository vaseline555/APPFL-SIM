...
from appfl_sim.privacy import laplace_mechanism_output_perturb
...
class ICEADMMTrainer(BaseTrainer):
    ...
    def _iceadmm_step(self, coefficient, global_state):
        ...
        """
        for name, param in self.model.named_parameters():
            self.primal_states[name] = self.primal_states[name].to(
                self.train_configs.device
            )
            self.dual_states[name] = self.dual_states[name].to(
                self.train_configs.device
            )
