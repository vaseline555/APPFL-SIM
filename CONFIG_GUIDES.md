...
- `loss_kwargs` (default: `{}`): keyword arguments forwarded to the loss constructor, e.g. `label_smoothing`.
- `configs` (default: `{}`): legacy alias for `loss_kwargs` kept for backward compatibility.
